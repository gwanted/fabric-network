...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ulaw.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ulaw.com
    docker cp -a orderer.ulaw.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ulaw.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ulaw.com

    for PEER in peer0.org1.ulaw.com peer1.org1.ulaw.com peer0.org2.ulaw.com peer1.org2.ulaw.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.ulaw.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.ulaw.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.ulaw.com`` & ``peer0.org2.ulaw.com``).  Secondly, we point to
